...
/// 判断文件是否需要版权检查（根据文件扩展名）
fn needs_copyright_check(file_path: &str) -> bool {
...
pub async fn check_copyright_with_ai(config: &crate::config::Config) -> Result<CopyrightCheckResult> {
    ...
    for file in &files {
        // 跳过不需要版权检查的文件类型
        if !needs_copyright_check(&file.path) {
            debug!("跳过不需要版权检查的文件: {} (类型: {})", file.path, file.file_type);
            continue;
        }
        
        debug!("使用 AI 检查文件: {} (类型: {})", file.path, file.file_type);
        
        // 构建 AI prompt
        let prompt = build_copyright_check_prompt(&file.content, &file.file_type, current_year);
...
/// 构建版权检查的 AI prompt
fn build_copyright_check_prompt(file_content: &str, file_type: &str, current_year: i32) -> String {
    format!(
        r#"请检查以下文件内容中的版权声明。
...
- 这是 {} 代码文件，请根据该语言的特点检查版权声明"#,
        file_type, current_year, current_year, file_content, file_type
    )
}
